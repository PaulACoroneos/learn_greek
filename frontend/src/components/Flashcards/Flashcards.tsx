  ...
  if (flashcards.length === 0) {
    return (
      <div className="text-center py-16 text-gray-400">
        ...
        <p className="text-sm mt-2">Words you miss during practice or save from reading appear here.</p>
      </div>
    )
  }
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <button onClick={() => setReviewing(false)} className="text-sm text-blue-600 hover:underline">
            ← Back to list
          </button>
          <span className="text-sm text-gray-500">{index + 1} / {flashcards.length}</span>
        </div>
        <div
        ...
        >
          {!flipped ? (
            <>
              ...
              {current.transliteration && <p className="text-gray-400 italic">{current.transliteration}</p>}
            ...
            <>
              ...
              {current.example && (
                <p className="text-sm text-gray-500 italic mt-3 border-t pt-3">e.g. {current.example}</p>
              )}
            </>
          )}
        </div>
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <p className="text-gray-600">
          <span className="font-semibold text-blue-700">{flashcards.length}</span> flashcard{flashcards.length !== 1 ? 's' : ''}
        </p>
        <div className="flex gap-3">
          <button
            onClick={() => { setIndex(0); setFlipped(false); setReviewing(true) }}
      ...
      <ul className="space-y-3">
        {flashcards.map((fc) => (
          <li key={fc.id} className="bg-white rounded-xl px-5 py-4 shadow-sm border border-gray-100 flex items-start justify-between gap-4">
            <div>
              <p className="text-xl font-semibold text-blue-700">{fc.greek}</p>
              {fc.transliteration && <p className="text-sm text-gray-400 italic">{fc.transliteration}</p>}
              <p className="text-gray-700 mt-1">{fc.english}</p>
              {fc.example && <p className="text-sm text-gray-400 italic mt-1">{fc.example}</p>}
              <p className="text-xs text-gray-300 mt-2">Reviewed {fc.reviewCount} time{fc.reviewCount !== 1 ? 's' : ''}</p>
            </div>
            <button
