...
describe('useFlashcards', () => {
  beforeEach(() => { lsMock.clear(); vi.clearAllMocks() })
  ...
  it('adds a flashcard', () => {
    const { result } = renderHook(() => useFlashcards())
    act(() => { result.current.addFlashcard('Γεια', 'Hello') })
  ...
  it('returns false for duplicate', () => {
    ...
    act(() => { r1 = result.current.addFlashcard('Γεια', 'Hello') })
    act(() => { r2 = result.current.addFlashcard('Γεια', 'Hello') })
    expect(r1!).toBe(true)
  ...
  it('removes a flashcard', () => {
    ...
    act(() => { result.current.removeFlashcard(id) })
    expect(result.current.flashcards).toHaveLength(0)
  })
  ...
  it('marks reviewed and increments count', () => {
    ...
    act(() => { result.current.addFlashcard('Γεια', 'Hello') })
    const id = result.current.flashcards[0].id
    act(() => { result.current.markReviewed(id) })
  ...
  it('clears all flashcards', () => {
    const { result } = renderHook(() => useFlashcards())
    act(() => { result.current.addFlashcard('Γεια', 'Hello'); result.current.addFlashcard('Ευχαριστώ', 'Thank you') })
