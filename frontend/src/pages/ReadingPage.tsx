  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <div>
        ...
        <p className="text-gray-500">Read Greek passages and click words you don't know to save them.</p>
      </div>
      <div className="flex gap-3 flex-wrap">
        {PASSAGES.map((p, i) => (
          <button key={p.id} onClick={() => setPidx(i)} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${i === pidx ? 'bg-green-600 text-white' : 'bg-white border border-gray-300 text-gray-600 hover:border-green-400'}`}>
            {p.title}
          </button>
        ))}
      </div>
      <ReadingMode key={passage.id} title={passage.title} words={passage.words as ReadingWord[]} onAddFlashcard={addFlashcard} />
    </div>
  )
}
