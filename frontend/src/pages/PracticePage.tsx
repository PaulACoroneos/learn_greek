...
const WB_EXERCISES = [
  {
    id: 'wb1', prompt: 'Good morning', promptTranslation: 'Select the correct Greek word',
    wordOptions: [
  ...
  {
    id: 'wb2', prompt: 'I would like water, please.', promptTranslation: 'Select words to form the sentence',
    wordOptions: [
  ...
  {
    id: 'wb3', prompt: 'Where is the bathroom?', promptTranslation: 'Select the correct phrase',
    wordOptions: [
...
const FT_EXERCISES = [
  { id: 'ft1', prompt: 'Hello', promptTranslation: 'Type the Greek translation', expectedAnswer: 'Γεια', hint: 'Starts with Γ', missing: [{ greek: 'Γεια', english: 'Hello' }] },
  { id: 'ft2', prompt: 'Thank you', promptTranslation: 'Type the Greek translation', expectedAnswer: 'Ευχαριστώ', hint: '9 letters, starts with Ε', missing: [{ greek: 'Ευχαριστώ', english: 'Thank you' }] },
  { id: 'ft3', prompt: 'Good night', promptTranslation: 'Type the Greek for "Good night"', expectedAnswer: 'Καληνύχτα', hint: 'Starts with Καλη…', missing: [{ greek: 'Καληνύχτα', english: 'Good night' }] },
]
...
export default function PracticePage() {
  ...
  function switchMode(m: Mode) { setMode(m); setIdx(0); setScore({ correct: 0, total: 0 }) }
  function nextEx() { setIdx((i) => i + 1) }
  function reset() { setIdx(0); setScore({ correct: 0, total: 0 }) }
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="flex gap-3">
        {(['word-bubbles', 'free-typing'] as Mode[]).map((m) => (
          <button key={m} onClick={() => switchMode(m)} className={`px-5 py-2 rounded-full text-sm font-semibold transition-colors ${mode === m ? 'bg-blue-600 text-white' : 'bg-white border border-gray-300 text-gray-600 hover:border-blue-400'}`}>
      ...
      {!done ? (
        <div className="space-y-4">
          <div className="flex items-center justify-between text-sm text-gray-500">
            <span>Exercise {idx + 1} of {exercises.length}</span>
            <span>Score: {score.correct}/{score.total}</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-1.5">
            <div className="bg-blue-500 h-1.5 rounded-full transition-all" style={{ width: `${(idx / exercises.length) * 100}%` }} />
          </div>
          ...
          <div className="flex justify-end">
            <button onClick={nextEx} className="px-6 py-2 text-sm text-blue-600 border border-blue-300 rounded-full hover:bg-blue-50">
        ...
        <div className="text-center py-12 bg-white rounded-2xl shadow-sm border border-blue-100">
          ...
          <p className="text-gray-500 text-lg mb-6">You got <span className="text-blue-600 font-bold">{score.correct}/{score.total}</span> correct</p>
          <button onClick={reset} className="px-8 py-3 bg-blue-600 text-white rounded-full font-semibold hover:bg-blue-700 shadow">Practice Again</button>
        </div>
      )}
    </div>
  )
